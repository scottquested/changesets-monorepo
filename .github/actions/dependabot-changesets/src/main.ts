...
export async function run(): Promise<void> {
  try {
    ...
    const packageName =
      core.getInput("package-name", { required: false }) || repo;
    const updateType =
      core.getInput("update-type", { required: false }) ?? "patch";
    const gitUser =
      core.getInput("git-user", { required: false }) ??
      dependabotCommitter.name;
    const gitEmail =
      core.getInput("git-email", { required: false }) ??
      dependabotCommitter.email;
    ...
    for await (const file of listFiles.data) {
      if (file.filename.includes("package.json")) {
        ...
        const content = Buffer.from(
          (packageJson.data as { content: string }).content,
          "base64"
        ).toString();
    ...
    for (const update of updates) {
      ...
      const existingChangeset = await readFile(changesetPath, "utf-8").catch(
        () => undefined
      );
      ...
      if (existingChangeset === newChangeset) {
        core.info(
          `⏭️ No changeset needed for ${update.package} (${update.from} -> ${update.to})`
        );
        continue;
      }
      ...
      newUpdates++;
      core.info(
        `✅ Created changeset for ${update.package} (${update.from} -> ${update.to}))`
      );
    }
